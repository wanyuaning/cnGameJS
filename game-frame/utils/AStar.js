class AStar {
    ...
    isInOpen(point) {
        return !!this.keyMapOpen[point.key];
    }
    isInClose(point) {
        return !!this.keyMapClose[point.key];
    }
    getPointWithMinF() {
        //获取开始列表中F值最小的结点
        var minPoint;
    ...
    deleteFromOpen(point) {
        delete this.keyMapOpen[point.key];
    }
    savePath(point) {
        // 保存路径
        this.path.unshift(point);
        if (point.parent) this.savePath(point.parent);
    }
    isWall(point, wallValueArr) {
        return !!wallValueArr[point.type];
    }
    ...
    checkPoint(point, parent) {
        this.isWall(point, this.wallValueArr) ? this.addToClose(point) : this.checkPoint2(point, parent);
    }
    handle(point) {
        // 寻路
        var x = point.x,
            ...
            _y = y - 1,
            y_ = y + 1,
            p1 = this.pointsMap[_x + "_" + _y],
            p2 = this.pointsMap[x + "_" + _y],
            p3 = this.pointsMap[x_ + "_" + _y],
            p4 = this.pointsMap[_x + "_" + y],
            p5 = this.pointsMap[x_ + "_" + y],
            p6 = this.pointsMap[_x + "_" + y_],
            p7 = this.pointsMap[x + "_" + y_],
            p8 = this.pointsMap[x_ + "_" + y_];
        ...
        if (point != this.endPoint) {
            ...
            this.handle(nextPoint); //递归重复上述操作
        } else {
            this.savePath(point); //如果当前结点是终点结点并且该结点被添加到关闭列表，则保存路径，寻路结束
        }
    }
    init(startPointPos, endPointPos) {
        this.keyMapOpen = {};
        this.keyMapClose = {};
        this.path = [];
    ...
    getPath(startPointPos, endPointPos) {
