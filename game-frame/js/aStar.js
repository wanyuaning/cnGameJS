...
var aStarManager = (function () {
    ...
    var addToClose = function (point) {
        if (isInOpen(point)) {
            //如果在开始列表中，则把结点从开始列表中删除，再添加到关闭列表中
    ...
    var handle = function (point) {
        ...
        for (var i = beginY; i <= endY; i++) {
            ...
            for (var j = beginX; j <= endX; j++) {
                if (pointsArr[i] && pointsArr[i][j] && !isInClose(pointsArr[i][j])) {
                    //如果该位置结点存在并且不是传入的结点

                    if (!isWall(pointsArr[i][j], this.wallValueArr)) {
                        //不是障碍物
                        ...
                        if (isInOpen(pointsArr[i][j])) {
                            //如果在开始列表，则根据传入的指定点计算是否改变G值
                            if (pointsArr[i][j].G > G) {
                                //如果该结点的G值大于由传入结点计算出的G值，则该结点父指针指向传入结点
                                pointsArr[i][j].G = G;
                                pointsArr[i][j].updateF();
                                pointsArr[i][j].parent = point;
                            }
                        } else if (!isInClose(pointsArr[i][j])) {
                            //如果不在开始列表也不在关闭列表，则根据传入的指定点计算该点G值，并把该结点父节点指针指向传入结点
                            pointsArr[i][j].G = G;
