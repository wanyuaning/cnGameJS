...
(function (win, undefined) {
    ...
    var _cnGame = {
        ...
        register: function (nameSpace, func) {
            ...
            for (var i = 0, len = nsArr.length; i < len; i++) {
                typeof parent[nsArr[i]] == "undefined" &&
                    (parent[nsArr[i]] = {});
    ...
    cnGame.register("cnGame", function (cg) {
        ...
        var resourceLoad = function (self, type) {
            return function () {
                ...
                self.loadedPercent = Math.floor(
                    (self.loadedCount / self.sum) * 100
                );
            ...
            start: function (gameObj, options) {
                ...
                if (cg.core.isArray(srcArr)) {
                    ...
                    for (var i = 0, len = srcArr.length; i < len; i++) {
                        var path = srcArr[i];
                        var suffix = srcArr[i].substring(
                            srcArr[i].lastIndexOf(".") + 1
                        );
                        var type = file_type[suffix];
                        if (type == "image") {
                            this.loadingImgs[path] = new Image();
                            cg.core.bindHandler(
                                this.loadingImgs[path],
                                "load",
                                resourceLoad(this, type)
                            );
                            ...
                            cg.core.bindHandler(
                                this.loadingAudios[path],
                                "canplay",
                                resourceLoad(this, type)
                            );
                            this.loadingAudios[path].onload = resourceLoad(
                                this,
                                type
                            );
    ...
    cnGame.register("cnGame.shape", function (cg) {
        ...
        text.prototype = {
            ...
            draw: function () {
                ...
                !cg.core.isUndefined(this.textBaseline) &&
                    (context.textBaseline = this.textBaseline);
                !cg.core.isUndefined(this.textAlign) &&
                    (context.textAlign = this.textAlign);
                !cg.core.isUndefined(this.maxWidth) &&
                    (context.maxWidth = this.maxWidth);
                if (this.isFill) {
                    context.fillStyle = this.style;
                    this.maxWidth
                        ? context.fillText(
    ...
    cnGame.register("cnGame.collision", function (cg) {
        ...
        this.col_Point_Rect = function (pointX, pointY, rectObj) {
            return (
                (pointX >= rectObj.x && pointX <= rectObj.right) ||
                (pointY >= rectObj.y && pointY <= rectObj.bottom)
            );
        };
        /**
         * 矩形和矩形间的碰撞
        ...
        this.col_Between_Rects = function (rectObjA, rectObjB) {
            return (
                ((rectObjA.right >= rectObjB.x &&
                    rectObjA.right <= rectObjB.right) ||
                    (rectObjA.x >= rectObjB.x &&
                        rectObjA.x <= rectObjB.right)) &&
                ((rectObjA.bottom >= rectObjB.y &&
                    rectObjA.bottom <= rectObjB.bottom) ||
                    (rectObjA.y <= rectObjB.bottom &&
                        rectObjA.bottom >= rectObjB.y))
            );
        };
        /**
        ...
        this.col_Point_Circle = function (pointX, pointY, circleObj) {
            return (
                Math.pow(pointX - circleObj.x, 2) +
                    Math.pow(pointY - circleObj.y, 2) <
                Math.pow(circleObj.r, 2)
            );
        };
        /**
         * 圆形和圆形间的碰撞
        ...
        this.col_between_Circles = function (circleObjA, circleObjB) {
            return (
                Math.pow(circleObjA.x - circleObjB.x, 2) +
                    Math.pow(circleObjA.y - circleObjB.y, 2) <
                Math.pow((circleObjA.r + circleObjB).r, 2)
            );
        };
    });
    ...
    cnGame.register("cnGame", function (cg) {
        ...
        map.prototype = {
            ...
            draw: function (options) {
                ...
                var img;
                for (
                    var i = beginY, ylen = beginY + row * cellSize[1];
                    i < ylen;
                    i += cellSize[1]
                ) {
                    //根据地图矩阵，绘制每个方格
                    ...
                    for (
                        var j = beginX,
                            xlen =
                                beginX +
                                mapMatrix[currentRow].length * cellSize[0];
                        j < xlen;
                        j += cellSize[0]
                    ) {
                        ...
                        cg.context.strokeText(
                            currentCol + "/" + currentRow,
                            currentCol * cellSize[0] + 12,
                            currentRow * cellSize[1] + 25
                        );
                    }
                }
            },
            ...
            getPosValue: function (x, y) {
                ...
                if (
                    !isUndefined(this.mapMatrix[y]) &&
                    !isUndefined(this.mapMatrix[y][x])
                ) {
